...
MODELO = "gemini-1.5-flash"  # Actualizado a un modelo estable
...
def analizar_ingesta(imagen_bytes=None, texto_usuario=None, perfil_usuario=None):
    ...
    # Si hay perfil de usuario, personalizar el consejo
    if perfil_usuario:
        contexto += f"""
...
if st.session_state.usuario_actual:
    ...
    if not base_datos.verificar_expediente(usuario):
        ...
        with tab2:
            ...
            col1, col2 = st.columns(2)
            with col1:
                # Pre-llenamos con el objetivo del perfil por defecto
                index_obj = ["Perdida de Grasa", "Ganar Músculo", "Mantenimiento", "Rendimiento Deportivo"].index(datos_perfil['objetivo']) if datos_perfil['objetivo'] in ["Perdida de Grasa", "Ganar Músculo", "Mantenimiento", "Rendimiento Deportivo"] else 0
                
                meta = st.selectbox("Objetivo del Ciclo", ["Perdida de Grasa", "Ganar Músculo", "Fuerza Pura", "Resistencia", "Recomposición"], index=0)
